...
# decorator to disable evernote connection on create instance of Kurippu
def GeekNoneDBConnectOnly(func):
    def wrapper(*args, **kwargs):
        Kurippu.skipInitConnection = True
        return func(*args, **kwargs)
    return wrapper

class Kurippu(object):
    ...
    def getStorage(self):
        if Kurippu.storage:
            return Kurippu.storage

        Kurippu.storage = Storage()
        return Kurippu.storage

    def getUserStore(self):
        if Kurippu.userStore:
            return Kurippu.userStore

        userStoreHttpClient = THttpClient.THttpClient(self.userStoreUri)
        userStoreProtocol = TBinaryProtocol.TBinaryProtocol(userStoreHttpClient)
        Kurippu.userStore = UserStore.Client(userStoreProtocol)
    ...
    def getNoteStore(self):
        if Kurippu.noteStore:
            return Kurippu.noteStore

        noteStoreUrl = self.getUserStore().getNoteStoreUrl(self.authToken)
        noteStoreHttpClient = THttpClient.THttpClient(noteStoreUrl)
        noteStoreProtocol = TBinaryProtocol.TBinaryProtocol(noteStoreHttpClient)
        Kurippu.noteStore = NoteStore.Client(noteStoreProtocol)
